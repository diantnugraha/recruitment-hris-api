# Employee Request PDF Template Cleanup — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Fix the Employee Request PDF template so text values don't collide with lines, values use regular (non-bold) font, Lexical JSON content extracts correctly, and rows expand dynamically for long text.

**Architecture:** Single-file refactor of `src/services/employeeRequestPdfService.ts`. Rewrite the Lexical text extractor to handle paragraph/list/listitem nodes and strip HTML. Rewrite the PDF rendering function to remove dotted lines, use `FONT_REGULAR` for values, position values below labels, and calculate dynamic row heights via PDFKit's `heightOfString`.

**Tech Stack:** Node.js, TypeScript, PDFKit v0.18.0

---

### Task 1: Rewrite Lexical text extractor and add HTML strip helper

**Files:**
- Modify: `src/services/employeeRequestPdfService.ts:64-99`

- [ ] **Step 1: Replace the text extraction code (lines 64–99) with the new implementation**

Replace everything from `// Lexical JSON to plain text extractor` (line 64) through the `extractTextLines` export (line 99) with:
...
Run: `cd /Users/diantnugraha/Documents/Documents/Development/recruitment-hris-api && npx tsc --noEmit src/services/employeeRequestPdfService.ts`

Expected: no errors (or only errors from other parts of the file, not the extraction code)

- [ ] **Step 3: Commit**

```bash
cd /Users/diantnugraha/Documents/Documents/Development/recruitment-hris-api
git add src/services/employeeRequestPdfService.ts
git commit -m "fix(pdf): rewrite Lexical text extractor to handle lists, paragraphs, and HTML"
```

---

### Task 2: Rewrite `generateEmployeeRequestPdf` — complete rendering overhaul

**Files:**
- Modify: `src/services/employeeRequestPdfService.ts:101-609`

This replaces the entire `generateEmployeeRequestPdf` function. The changes:
- Add `getTextHeight()` helper for dynamic row sizing
- Add `PAD` constant (4px cell padding)
- Remove ALL `drawDottedLine` calls except in signature section
- Change ALL value `font: FONT_BOLD` → removed (defaults to `FONT_REGULAR`)
- Position values BELOW bilingual labels (at `curY + 3 + LABEL_H`)
- Calculate dynamic row heights using `getTextHeight` before drawing cells
- Add `width` constraints on all value text to prevent overflow

- [ ] **Step 1: Replace the entire `generateEmployeeRequestPdf` function (lines 101–609)**

Replace from `export function generateEmployeeRequestPdf` through the closing `}` of the function with:
...
export function generateEmployeeRequestPdf(data: EmployeeRequestPdfData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    try {
      ...
      drawText(String(data.numberOfRequired), tableX + basicCol1 + basicCol2 + PAD, curY + 3 + LABEL_H, { size: 7 })
      ...
      drawText(data.requestedDate, tableX + 80, curY + 3 + LABEL_H, { size: 7 })

      drawBilingual('Tanggal Dibutuhkan', 'Date Required', tableX + basicCol1 + PAD, curY + 3, { size: 6.5 })
      drawText(data.dateRequired, tableX + basicCol1 + 80, curY + 3 + LABEL_H, { size: 7 })

      drawBilingual('Level Posisi', 'Position Level', tableX + basicCol1 + basicCol2 + PAD, curY + 3, { size: 6.5 })
      drawText(data.positionLevel, tableX + basicCol1 + basicCol2 + PAD, curY + 3 + LABEL_H, { size: 7 })
      ...
      const typeCol4 = fullW - typeCol1 - typeCol2 - typeCol3 // Gender
      ...
      // Dynamic height based on reason text
      ...
      drawText(data.minimumAge, qcX + 65, curY + 3, { size: 7 })

      drawBilingual('Usia Maksimal', 'Maximum Age', qrX, curY + 2, { size: 6.5 })
      drawText(data.maximumAge, qrX + 65, curY + 3, { size: 7 })
      ...
      drawText(data.educational, qcX + 60, curY + 3, { size: 7 })
      ...
      for (let i = 0; i < 5; i++) {
        ...
        // Row with label (i===1) needs space for bilingual label + content
        const hasLabel = i === 1
...
Run: `cd /Users/diantnugraha/Documents/Documents/Development/recruitment-hris-api && npx tsc --noEmit`
...
cd /Users/diantnugraha/Documents/Documents/Development/recruitment-hris-api
git add src/services/employeeRequestPdfService.ts
git commit -m "fix(pdf): clean up Employee Request PDF layout — remove dotted lines, fix fonts, dynamic row heights"
```

---

### Task 3: Build and verify PDF output
...
Run: `cd /Users/diantnugraha/Documents/Documents/Development/recruitment-hris-api && npm run build`

Expected: successful build with no errors

- [ ] **Step 2: Start dev server and test the PDF endpoint**

Run: `cd /Users/diantnugraha/Documents/Documents/Development/recruitment-hris-api && npm run dev`

Then open in browser: `http://localhost:3001/v1/employee-request/{id}/pdf` (use a valid employee request ID)

**Verify in the PDF output:**
- [ ] Values use regular weight font (not bold)
- [ ] No dotted lines on value fields (only in signature section)
- [ ] Values positioned below bilingual labels with visible gap
- [ ] No garbled characters in Job Descriptions section
- [ ] No HTML tags (`<ol>`, `<li>`, etc.) visible
- [ ] Long text wraps and rows expand to fit
- [ ] Text stays within cell boundaries
- [ ] Signature section dotted lines still present under names
- [ ] All content on A4 page
...
cd /Users/diantnugraha/Documents/Documents/Development/recruitment-hris-api
git add src/services/employeeRequestPdfService.ts
git commit -m "fix(pdf): fine-tune spacing after visual verification"
```
